...
// URL: https://www.beecrowd.com.br/judge/en/problems/view/3064
...
int main(){
	...
	while(scanf("%d", &N)!=EOF){
		...
		sort(++bands.begin(), bands.end(), radial);
		
		/*rep(i, 1, bands.size()-1){
			debugPP(bands[i]);
			//debug(ccw(pivot, bands[i], bands[i+1]));
		}*/
		
		int ans = 0; cnt++;
		rep(i, 1, bands.size()){
			ans = max(ans, solve(0, i)+2);
		}
