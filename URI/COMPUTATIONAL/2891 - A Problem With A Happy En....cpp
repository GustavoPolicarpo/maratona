...
// Name: A Problem With A Happy En...
// Level: 
// Category: COMPUTATIONAL
// URL: https://www.beecrowd.com.br/judge/en/problems/view/2891
...
int main(){
	...
	while(42){
		...
		do{
			vector <point> T;
			rep(i, 0, 4) T.pb(pts[idx[i]]);
			sort(T.begin(), T.end());
			pivot = T[0];
			sort(++T.begin(), T.end(), radial_lt);
			int dir = ccw(T[0], T[1], T[2]);
			bool valid = true;
			rep(i, 1, T.size()){
				int rid = ccw(T[i], T[(i+1)%T.size()], T[((i+1)%T.size()+1)%T.size()]);
				if(rid!=dir){
					valid = false;
					break;
				}
