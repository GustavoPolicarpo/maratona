...
// URL: https://www.beecrowd.com.br/judge/en/problems/view/1693
...
int main()
{
	while(scanf("%d %d %d %d", &R, &H, &V, &G)!=EOF){
		...
		a=-G/2.0, c=H;
		double ini=0.0, fim=91.0, u1=(ini+fim)/2.0, u2=u2=min(u1+1.0, fim-0.01);
		while(fim-ini>0.001){
			//cout << fim << " " << ini << endl; 
			teta=(u1*M_PI/180.0);
			b=V*sin(teta);
			...
			if(a1<=a2){
				ini=u1, fim=fim;
				u1=(ini+fim)/2.0, u2=min(u1+1.0, fim-0.01);
