...
// Level: 8
// Category: GRAPH
// URL: https://www.beecrowd.com.br/judge/en/problems/view/1476
...
void dfs(int u, int last){	
	rep(i, 0, adj[u].size()){
		...
		peso[v][0]=adj[u][i].S;
		rep(j, 1, LOG) peso[v][j]=min(peso[v][j-1], peso[pai[v][j-1]][j-1]);
...
int LCA(int a, int b){
	...
	for(int i=LOG-1; i>=0; i--){
		if(pai[a][i]!=pai[b][i]){
			ret=min(ret, min(peso[a][i], peso[b][i]));
			a=pai[a][i], b=pai[b][i];		
		}
	}
	ret=min(ret, min(peso[a][0], peso[b][0]));
