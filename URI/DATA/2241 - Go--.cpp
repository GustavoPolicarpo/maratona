...
// Level: and
// Category: DATA
// URL: https://www.beecrowd.com.br/judge/en/problems/view/2241
...
int lower(int i, int j, int id){
	...
	while(fim-ini>1){
		if(Q(i, j, use, id)){
			fim=use, use=(ini+fim)/2 + (((ini+fim))%2==1);
		}
		else{
			ini=use, use=(ini+fim)/2 + (((ini+fim))%2==1);
		}
	}
	return use;
}
...
int upper(int i, int j, int id, int aa){
	...
	//
	while(fim-ini>1){
	//cout << "     " << ini << " " << use << " " << fim  << " " <<Q(i, j, use, id)  << endl;
