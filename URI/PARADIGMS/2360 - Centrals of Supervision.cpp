...
// Level: 10
// Category: PARADIGMS
// URL: https://www.beecrowd.com.br/judge/en/problems/view/2360
...
ll solve(int u, int c, int cc){
    ...
    if(adj[u].size()==0){ 
        ...
        if(cc==0) ret=(solve(v, 0, 0)*solve(adj2[u][0], min(c, 1), 0))%mod+
        			   (solve(v, 0, 0)*solve(adj2[u][0], min(c, 1), 1))%mod+
        			    (solve(v, 0, 1)*solve(adj2[u][0], min(c, 1), 0))%mod+
        			     (solve(v, 0, 1)*solve(adj2[u][0], min(c, 1), 1))%mod;
        if(cc==1) ret=(solve(v, min(c+1, 1), 0)*solve(adj2[u][0], min(c, 1), 0))%mod+
        			   (solve(v, min(c+1, 1), 0)*solve(adj2[u][0], min(c, c), 1))%mod+
        			    (solve(v, min(c+1, 2), 1)*solve(adj2[u][0], min(c, 1), 0))%mod+
        			     (solve(v, min(c+1, 2), 1)*solve(adj2[u][0], min(c, c), 1))%mod;
    }
