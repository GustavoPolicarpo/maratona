...
// Level: 4
// Category: BEGINNER
// URL: https://www.beecrowd.com.br/judge/en/problems/view/3053
...
int main(){
	...
	int N; cin >> N;
	int pos[3]; pos[0]=pos[1]=pos[2]=0;
	char op; cin >> op; pos[op-'A'] = 1;
	
	rep(i, 0, N){
		int po; cin >> po;
		if(po==1) swap(pos[0], pos[1]);
		if(po==2) swap(pos[2], pos[1]);
		if(po==3) swap(pos[0], pos[2]);
	}
